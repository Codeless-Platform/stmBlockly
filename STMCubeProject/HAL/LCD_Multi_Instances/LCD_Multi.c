...
void lcd_init(LCD_t *LCD_Config) {
	...
	if (LCD_Config->LCD_Size == LCD_2x16) {
		LCD_Config->cols = 16;
		LCD_Config->rows = 2;
	} else if (LCD_Config->LCD_Size == LCD_4x16) {
		LCD_Config->cols = 16;
		LCD_Config->rows = 4;
	} else if (LCD_Config->LCD_Size == LCD_4x20) {
		LCD_Config->cols = 20;
		LCD_Config->rows = 4;
	}
...
void lcd_Send_Char(unsigned char character, LCD_t *LCD_Config) {
	...
	if (LCD_Config->count == LCD_Config->cols) {
		lcd_GOTO_XY(1, 0, LCD_Config);
	}	// if both lines are full clear and start over.
	else if (LCD_Config->count == (LCD_Config->cols * LCD_Config->rows)) {
		lcd_Clear_Screen(LCD_Config);
		lcd_GOTO_XY(0, 0, LCD_Config);
		LCD_Config->count = 0;
	}else if ((LCD_Config->LCD_Size == LCD_4x20)
			|| (LCD_Config->LCD_Size == LCD_4x16)) {
		if (LCD_Config->count == (LCD_Config->cols * 2)) {
			lcd_GOTO_XY(2, 0, LCD_Config);
		} else if (LCD_Config->count == (LCD_Config->cols * 3)) {
...
void lcd_GOTO_XY(unsigned char row, unsigned char col, LCD_t *LCD_Config) {
	...
	if (LCD_Config->LCD_Size == LCD_4x20) {
		if (row == 2) {
			if ((col < LCD_Config->cols) && (col >= 0))
				lcd_Send_Command(CURSOR_THIRD_LINE_20 + col, LCD_Config);
		} else if (row == 3) {
			if ((col < LCD_Config->cols) && (col >= 0))
				lcd_Send_Command(CURSOR_FOURTH_LINE_20 + col, LCD_Config);
		}
	} else if (LCD_Config->LCD_Size == LCD_4x16) {
		if (row == 2) {
			if ((col < LCD_Config->cols) && (col >= 0))
				lcd_Send_Command(CURSOR_THIRD_LINE_16 + col, LCD_Config);
		} else if (row == 3) {
			if ((col < LCD_Config->cols) && (col >= 0))
				lcd_Send_Command(CURSOR_FOURTH_LINE_16 + col, LCD_Config);
		}
	}
